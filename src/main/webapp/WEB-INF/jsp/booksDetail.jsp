    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <p class="mb-4">세모 도서관의 도서를 검색하고, 상세페이지에서 대출신청을 할 수 있습니다.</p>

                <!-- DataTales Example -->
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">도서 상세조회</h6>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <form method="post" name="frm">
                                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tr>
                                        <th>도서명</th>
                                        <td id="bookName">${book.book_name}</td>
                                    </tr>
                                    <tr>
                                        <th>저자</th>
                                        <td>${book.writer}</td>
                                    ...
                                    <tr>
                                        <th>평점</th>
                                        <td>
                                            <c:if test="${ empty score}">0.0 / 5.0</c:if>
                                            <c:if test="${ not empty score}">${ score.score} / 5.0</c:if>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>한줄평</th>
                                        <td>
                                            <ul class="reviews">
                                                <c:if test="${ empty reviews}">
                                                    <li class="reviewList">아직 한줄평이 없습니다. </li>
                                                </c:if>
                                    ...
                                    <tr>
                                        <th>대출 상태</th><input type="hidden" name="member_id" id="r_member_id" value="#">
                                        <c:if test="${ empty state or '반납완료' eq state.state}">
                                            <td>대출 가능 &nbsp; <input type="button" id="btn" value="대출 신청" onclick="insertRental(this.form)">
                                            </td>
                                        </c:if>
                                        <c:if test="${ '대출중' eq state.state or '반납처리중' eq state.state}">
                                            <td>대출 불가(대출중)<br>반납예상일: ${ state.return_deadline}
                                            </td>
                                        </c:if>
                                    </tr>
                                    </tbody>
...
<script>
    ...
    //로그인 세션의 member_id 보내기
    document.getElementById('r_member_id').value=memberId;

    function insertRental(frm) {
        var rentalCnt = parseInt(${rentalCnt});
        if (rentalCnt < 5){
            var isRental = confirm("현재 회원님이 대여 중인 도서는 "+rentalCnt+"권 입니다. 대출 신청하시겠습니까?")
            if (isRental) {
                alert("대출 신청이 완료되었습니다. 마이페이지에서 대출 현황을 확인하세요.")
                frm.action = "/bookRental";
                frm.submit();
            }
        } else {
            alert('이미 5권을 대출하였습니다. 한 회원 당 최대 5권까지 빌릴 수 있습니다.');
        }
    }
