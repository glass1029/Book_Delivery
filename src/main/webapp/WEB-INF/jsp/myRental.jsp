    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <p class="mb-4">::: 대여한 도서 목록 :::
                    <br>도서 대여기간은 14일 입니다.
                    <br> 대여 기간 내에 연장 신청은 2회(회당 7일 연장)까지 가능하며 연체 시에는 연장 불가합니다.
                    <br>책 제목에 마우스를 올리면 작가를 학인할 수 있습니다.</p>
                   ...
                   <%-- <div class="card-header py-3">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table data-order='[[2, "desc"]]' class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                <thead>
                                <tr>
                                    <th>책 제목</th>
                                    <%--<th>작가</th>--%>
                                ...
                                <c:if test="${ empty myRentalList}">
                                    <tr><td colspan="11">아직 대여한 도서가 없습니다. 지금 바로 서비스를 이용해보세요!</td></tr>
                                </c:if>
                                <c:if test="${ not empty myRentalList}">
                                    <c:forEach items="${ myRentalList}" var="myRental">
                                        <c:set var="today" value="<%=new java.util.Date()%>"/>
                                        <c:set var="deadline" value="${myRental.return_deadline}"/>
                                        <fmt:formatDate var="now" type="date" value="${today}" pattern="yyyy-MM-dd"/>
                                        <fmt:parseDate var="_to" value="${deadline}" pattern="yyyy-MM-dd"/>
                                        <fmt:formatDate var="to" type="date" value="${_to}" pattern="yyyy-MM-dd"/>

                                        <tr>
                                            <input type="hidden" id="rental_id" name="rental_id" value="${myRental.rental_id}">
                                            <td title = "${myRental.writer}">${myRental.book_name}</td>
                                            <td>${myRental.publisher}</td>
                                            <input type="hidden" value ="${myRental.writer}">
                                            <td>${myRental.rental_date}</td>
                                            <td>${to}</td>
                                            <td>${myRental.return_date}</td>
                                            <td><c:if test="${myRental.state == '대출중' and now > to}">연체중
                                                </c:if>
                                                <c:if test="${myRental.state == '대출중' and now <= to}">${myRental.state}
                                                </c:if>
                                                <c:if test="${myRental.state != '대출중'}">${myRental.state}</c:if>
                                            </td>

                                            <td>${myRental.is_extended}</td>
                                            <td id="btns" align="center"><%--상태 나타는 부분--%>
                                                <c:if test ="${myRental.state == '대출중'}">

                                                <input rId = "${myRental.rental_id}" mId="${myRental.member_id}" type="button" id="btn" value="반납" onclick="return_go(this)"></c:if>
                                                <c:if test ="${myRental.state == '반납처리중'}"> - </c:if>
                                                <c:if test ="${myRental.state == '반납완료'}"> - </c:if>

                                            <td id="btns">
                                                <c:choose>

                                                    <c:when test = "${myRental.is_extended >= 2}">
                                                        <a style="cursor:pointer;" onclick="alert('연장 가능 횟수 초과(최대 2회)');"> 불가 </a>
                                                    </c:when>
                                                    <c:when test = "${myRental.state !='대출중'}">
                                                        <a style="cursor:pointer;" onclick="alert('이미 반납을 요청하셨습니다.');"> 불가 </a>
                                                    </c:when>
                                                    <c:when test = "${ now > to}">
                                                        <a style="cursor:pointer;" onclick="alert('연체중에는 해당 도서 기간을 연장할 수 없습니다.');"> 불가 </a>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <input rId= "${myRental.rental_id}" mId="${myRental.member_id}" id="btn" type="button" value="연장" onclick="extend_go(this)">
...
<script>
    function extend_go(frm) {
        var isExtend = confirm("대여 기간을 연장하시겠습니까?");
        if(isExtend) {
            alert("연장은 회당 7일, 2회까지 가능합니다.");
            this.rId;
            this.mId;
            location.href = "/extendUpdate?rental_id="+frm.getAttribute('rId')+"&member_id="+frm.getAttribute('mId');
        }
    }
    function return_go(frm) {
        var isReturn = confirm("책을 반납하시겠습니까?");
        if(isReturn) {
            alert("반납신청 완료 - 도서가 도착하면 반납 완료됩니다.");
            this.rId;
            this.mId;
            location.href = "/returnUpdate?rental_id="+frm.getAttribute('rId')+"&member_id="+frm.getAttribute('mId');
        }
    }
</script>
