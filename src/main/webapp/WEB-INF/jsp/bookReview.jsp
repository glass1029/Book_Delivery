    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <c:if test="${ not empty review}">
                <%-- <tr>
                    <td>
                        <input type="hidden" name="communityId" value="${review.communityId }">
                    </td>
                </tr> --%>
            <tr>
                <th width="20%">도서제목</th>
                <td width="50%">${review.book_name}</td>
                <th>별점</th>
                <td>${review.score}</td>
            </tr>
            ...
            <tr>
                <th>글제목</th>
                <td colspan="3">
                    <input type="hidden" name="community_id" value="${review.community_id }">${review.title}</td>
            </tr>
            <tr>
                <th>한줄평</th>
            ...
            <tr>
                <th>내용</th>
                <td colspan="3" height="200px">${review.content}</td>
            </tr>
            ...
            <tfoot>
            <tr>
                <td colspan="5" id="btns">
                    <%--<input type="button" value="수정" onclick="fn_update(this.form)" id="btn">
                        <input type="button" value="삭제" onclick="fn_deleteCommunity(this.form)" id="btn">--%>
                    <input type="button" value="수정" onclick="if(${review.member_id} == sessionStorage.getItem('memberId'))
                        { fn_update(this.form) } else { alert('작성자만 수정할 수 있습니다.') }" id="btn">
                        <input type="button" value="삭제" onclick="if(${review.member_id} == sessionStorage.getItem('memberId'))
                        { fn_deleteCommunity(this.form) } else { alert('작성자만 삭제할 수 있습니다.') }" id="btn">
                </td>
            </tr>
            </tfoot>
                        </c:if>
           ...
           <%-- </form>
                            &lt;%&ndash;댓글란 시작&ndash;%&gt;
                <div class="comment" id="commentCommunity">
                    <hr>
                    <form method="post" name="frm">
                        <p>작성자:
                            <input type="text" id="mid" name="member_id" value="#" disabled>
                        </p>
                        <p>
                            <textarea name="content" rows="4" cols="100" required></textarea> &nbsp;&nbsp; <input type="button" id="btn" value="작성" onclick="insertComments(this.form)">
                        </p>
                        <input type="hidden" name="community_id" value="${review.community_id }">
                    </form>
                </div>
                ...
                <c:forEach var="comments" items="${ comments}">
                    <div class="comment">
                        <form name="frm" method="get" action="/commentsDelete" onsubmit="return confirm('삭제하시겠습니까?');">
                            <p>작성자: ${ comments.name} &nbsp;&nbsp;&nbsp;&nbsp;작성일: ${ comments.regdate}&nbsp;&nbsp;&nbsp;&nbsp;
                                <input type="submit" value="삭제" id="btn">
                            </p>
                            <p>${ comments.content}</p>
                            <input type="hidden" name="comments_id" value="${ comments.comments_id}">
                            <input type="hidden" name="community_id" value="${review.community_id }">
                            <hr>
                        </form>
                    </div>
...
<script>
function fn_deleteCommunity(frm){
	var delete1 = confirm
	if(delete1){
		alert("삭제하시겠습니까?")
		frm.action = "/deleteCommunity";
		frm.submit();
	}

}
function fn_update(frm){
	var update1 = confirm
	if(update1){
		alert("수정하시겠습니까?")
		var str = ${review.community_id}
		console.log(str);
		frm.action = "/modifyCommunity?community_id="+str;
		frm.submit();
	}
}


function insertComments(frm){
    frm.action = "/commentInsert";
    frm.submit();
}

/*     function insertData(frm){
        if(!document.frm.category.value){
            alert("말머리를 선택하세요");
            document.frm.category.focus();
            return false;
        }
        var isWrite = confirm("글을 등록하시겠습니까?")
        if(isWrite){
            frm.action = "/suggestInsert";
            frm.submit();
        }
    }
    function list_go(){
